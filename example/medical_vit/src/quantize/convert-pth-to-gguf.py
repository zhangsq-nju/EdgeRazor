...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the PyTorch checkpoint to convert. If not provided, will use pretrained weights from timm.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=None,
        help="Path to the output file. If not provided, will use default naming.",
    )
    ...
    # Output file name
    fname_out = args.output_path or f"./ggml-model-{['f32', 'f16'][args.ftype]}.gguf"
    os.makedirs(os.path.dirname(fname_out), exist_ok=True)

    # Load the pretrained model
    if args.ckpt_path:
        timm_model = timm.create_model(args.model_name, pretrained=False, num_classes=5)
        checkpoint = torch.load(args.ckpt_path, map_location="cpu")
        timm_model.load_state_dict(checkpoint)
        print(f"Loaded weights from {args.ckpt_path}")
    else:
        timm_model = timm.create_model(args.model_name, pretrained=True, num_classes=5)
        print(f"Loaded pretrained weights for {args.model_name} from timm")
