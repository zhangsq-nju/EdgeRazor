...
1. reshape to [out_channels, in_channels * kernel_height * kernel_width]
...
class QConv2d(nn.Conv2d):
    def __init__(
        ...
        if quant_config is None:
            raise ValueError("quant_config must be provided for QConv2d.")
    ...
    def _weight_quant(self, replace_self: bool = False) -> Tensor:
        ...
        # Need to reshape to [out_channels, in_channels * kernel_h * kernel_w] for quantization
        W = self.weight.data.clone()
        original_shape = W.shape
        
        # Reshape: flatten(1) ensures kernel_h * kernel_w are adjacent
        # Shape: [out_channels, in_channels, kernel_h, kernel_w] -> [out_channels, in_channels * kernel_h * kernel_w]
...
def copy_conv2d_to_qconv2d(
    conv: nn.Conv2d,
    qconv_cls: nn.Module,
    quant_config: QuantConfig = None
):
    """Copy Conv2d to quantized Conv2d (adjust according to your QConv2d implementation)"""
    # Adjust according to your QConv2d implementation
    qconv = qconv_cls(
