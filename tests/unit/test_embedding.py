...
class TestEmbeddingQuantize:
    ...
    def test_int2_embedding_roundtrip(self, vocab, hidden, indices):
        """INT2 embedding: index_select → unpack_int2 → dequant."""
        torch.manual_seed(42)
        w = torch.randn(vocab, hidden, dtype=torch.bfloat16)
...
class TestEmbeddingDequantOnly:
    ...
    def test_int2_single_row_dequant(self):
        """One row INT2: pack → unpack → dequant matches original."""
        row = torch.randn(1, 256, dtype=torch.bfloat16)
        qw, sc = quantize_weight_per_block_int2(row)
